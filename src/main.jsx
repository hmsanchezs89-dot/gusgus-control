...
function App(){
  ...
  const allSalesTotal = sales.reduce((s,v)=>s+Number(v.total_final || v.subtotal || 0),0)
  const allSalesCost = sales.reduce((s,v)=>s+Number(v.costo_total || 0),0)
  const allGrossBase = allSalesTotal - allSalesCost

  const weekSalesTotal = salesWeek.reduce((s,v)=>s+Number(v.total_final || v.subtotal || 0),0)
  const weekSalesCost = salesWeek.reduce((s,v)=>s+Number(v.costo_total || 0),0)
  const weekGrossBase = weekSalesTotal - weekSalesCost

  function reparto(base){
    const safe = Math.max(0, Number(base || 0))
    return {
      base: safe,
      reinversion: safe * 0.50,
      sueldo: safe * 0.30,
      gastosFijos: safe * 0.10,
      reserva: safe * 0.10,
    }
  }

  const repartoTotal = reparto(allGrossBase)
  const repartoSemana = reparto(weekGrossBase)
  ...
  return <div className="app">
    ...
    <nav className="nav">{[['dashboard','🏠 Inicio'],['panel','📊 Panel'],['caja','💵 Caja'],['ventas','📈 Ventas'],['productos','🍗 Productos'],['inventario','📦 Inventario'],['compras','🛒 Compras'],['mermas','⚠️ Mermas'],['gastos','💸 Gastos'],['sat','🧾 SAT']].map(([id,label])=><button key={id} onClick={()=>setTab(id)} className={tab===id?'active':''}>{label}</button>)}</nav>
    ...
    {tab==='panel'&&<main className="panel">
      <h2>Panel de reparto operativo</h2>
      <p className="muted">Cálculo: venta total - costo de productos = base para repartir.</p>

      <div className="panelSplit">
        <section className="splitBox">
          <h3>Acumulado total</h3>
          <div className="cards2">
            <div><span>Venta total</span><b>{money(allSalesTotal)}</b></div>
            <div><span>Costo total</span><b>{money(allSalesCost)}</b></div>
            <div><span>Base a repartir</span><b>{money(repartoTotal.base)}</b></div>
          </div>
          <div className="repartoGrid">
            <div><span>50% Reinversión</span><b>{money(repartoTotal.reinversion)}</b></div>
            <div><span>30% Sueldo</span><b>{money(repartoTotal.sueldo)}</b></div>
            <div><span>10% Gastos fijos</span><b>{money(repartoTotal.gastosFijos)}</b></div>
            <div><span>10% Libre / reserva</span><b>{money(repartoTotal.reserva)}</b></div>
          </div>
        </section>

        <section className="splitBox">
          <h3>Semana actual jueves-lunes</h3>
          <p className="muted">{week.start} → {week.end}</p>
          <div className="cards2">
            <div><span>Venta semana</span><b>{money(weekSalesTotal)}</b></div>
            <div><span>Costo semana</span><b>{money(weekSalesCost)}</b></div>
            <div><span>Base a repartir</span><b>{money(repartoSemana.base)}</b></div>
          </div>
          <div className="repartoGrid">
            <div><span>50% Reinversión</span><b>{money(repartoSemana.reinversion)}</b></div>
            <div><span>30% Sueldo</span><b>{money(repartoSemana.sueldo)}</b></div>
            <div><span>10% Gastos fijos</span><b>{money(repartoSemana.gastosFijos)}</b></div>
            <div><span>10% Libre / reserva</span><b>{money(repartoSemana.reserva)}</b></div>
          </div>
        </section>
      </div>

      <p className="notice">Nota: este panel reparte después de quitar costos de producto. Los impuestos/SAT se siguen mostrando en el apartado SAT.</p>
    </main>}
