...
function calcCart(items, channel) {
  ...
  const resico = subtotal * 0.0625
  const before = subtotal - cost - commission - iva - resico
  const reinvest = Math.max(0, before * 0.20)
  const profit = before - reinvest
  const locked = iva + resico + reinvest
  return { subtotal, cost, commission, iva, resico, reinvest, profit, locked }
}
...
function App() {
  ...
  const cartSummary = useMemo(() => calcCart(cart, channel), [cart, channel])
          ...
          <aside className="panel">
            ...
            {cart.map(i => (
              <div className="cartItem" key={i.id}>
                <div><b>{i.codigo} {i.nombre}</b><small>{money(price(i, channel))} c/u</small></div>
                <div>
                  <button onClick={() => setQty(i.id, -1)}>-</button>
                  <span>{i.qty}</span>
                  <button onClick={() => setQty(i.id, 1)}>+</button>
                  <button onClick={() => setCart(cart.filter(x => x.id !== i.id))}>×</button>
                </div>
              </div>
            ))}
            ...
            <button className="charge" onClick={charge} disabled={cart.length === 0 || loading}>COBRAR {money(cartSummary.subtotal)}</button>
          </aside>
        </main>
      )}
        ...
        <section className="panel">
          ...
          {sales.map(v => (
            <div className="row" key={v.id}>
              <span>{v.fecha_operativa}</span><span>{v.canal}</span><span>{v.metodo_pago}</span><b>{money(v.subtotal)}</b><b className="okText">{money(v.utilidad_real)}</b><b className="dangerText">{money(v.no_tocar)}</b>
            </div>
          ))}
        </section>
      )}
